...
public interface LoginRepository extends JpaRepository<EmployeeManage, Integer>{
	@Query(value="SELECT em FROM  EmployeeManage em WHERE em.number = :number and em.password =:password and em.employeeType.id=5")
	...
	int update(@Param("phone")String number,@Param("password")String password);
